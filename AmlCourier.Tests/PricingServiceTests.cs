...
public class PricingServiceTests
{
    ...
    public void Order_Cost_With_Single_Parcel_Is_Calculated(int length, int width, int height, decimal expectedCost)
    {
        var order = new Order();
        order.AddParcel(new Parcel(length, width, height));

        var costedOrder = PricingService.GetOrderCost(order);
        Assert.Equal(expectedCost, costedOrder.GetTotalCost());
    }
    ...
    public void Order_Cost_With_Single_Parcel_With_Speedy_Is_Calculated(int length, int width, int height, decimal expectedCost)
    {
        var order = new Order();
        order.AddParcel(new Parcel(length, width, height));
    ...
    public void Order_With_No_Parcels_Should_Cost_Zero()
    {
        ...
        Assert.Equal(0m, costedOrder.GetTotalCost());
    }
}